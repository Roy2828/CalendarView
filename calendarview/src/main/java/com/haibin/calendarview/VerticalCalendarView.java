...
public class VerticalCalendarView extends CalendarView {
    ...
    protected void init(Context context, @Nullable AttributeSet attrs) {
        ...
        boolean horizontal_item_line_visible = array.getBoolean(R.styleable.VerticalCalendarView_horizontal_item_line_visible,true);
        array.recycle();
        ...
        if(horizontal_item_line_visible){
            this.mWeekLine.setVisibility(View.VISIBLE);
        }else{
            this.mWeekLine.setVisibility(View.GONE);
        }
        //月视图
    ...
    public void addOnVerticalYearMonthChangeListener(final ChangeVerticalYearMonth changeVerticalYearMonth){
        monthRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(RecyclerView recyclerView, int newState){
                super.onScrollStateChanged(recyclerView, newState);
                yearMonthCalculation(recyclerView,changeVerticalYearMonth);
    ...
    void yearMonthCalculation(RecyclerView recyclerView,ChangeVerticalYearMonth changeVerticalYearMonth) {
            ...
            if (layoutManager instanceof LinearLayoutManager) {
                ...
                for (int i = firstItemPosition; i<=lastItemPosition; i++){
                    ...
                    if(rect.height()>height){
                        int month =   currentItemViewMonth(i);
                        int year =   currentItemViewYear(i);
                        changeVerticalYearMonth.yearMonth(year,month);
                    }
    ...
    public  int currentItemViewMonth(int position){
       int month = (position + mDelegate.getMinYearMonth() - 1) % 12 + 1;
        return month;
    }

    public  int currentItemViewYear(int position){
    ...
    public  interface ChangeVerticalYearMonth{
